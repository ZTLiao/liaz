...
class _NovelHorizontalReaderState extends State<NovelHorizontalReader>
  ...
  Future initText() async {
    ...
    fontHeight = chineseCharSize.height;
    Size englishCharSize = calcFontSize("z",
        fontSize: fontSize.toDouble(), lineHeight: lineHeight);
    Size symbolCharSize = calcFontSize(StrUtil.comma,
        fontSize: fontSize.toDouble(), lineHeight: lineHeight);
    Size spaceCharSize = calcFontSize(StrUtil.space,
        fontSize: fontSize.toDouble(), lineHeight: lineHeight);
    // 计算可渲染的最大行数
  ...
  static List<List<String>> splitText(
    ...
    for (var item in resultList) {
      ...
      if (newLineExp.hasMatch(item)) {
        chars.add(
          CharInfo(text: StrUtil.empty, width: 0, type: CharType.newline),
        );
        continue;
      }
      if (item == StrUtil.space) {
        chars.add(
          CharInfo(
            text: item,
